...
export const VerificationPage: React.FC = () => {
    ...
    const requestUserRegistration = async (email: string, username: string, password: string) => {
        try {
            ...
            navigate("/user/login", {
                state: {
                    registeredMessage: "You have successfully been registered. You can now login.",
                    prefillCredentials: { username, password }
                }
            });
        }
        catch (error: any) {
            navigate('/user/registration', {state: {"registrationDetails": {email, username, password}, errorRegisteringAfterVerification: error.message}});
        }
    }
