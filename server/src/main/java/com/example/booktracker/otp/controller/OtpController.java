package com.example.booktracker.otp.controller;
...
import com.example.booktracker.otp.request.OtpRequest;
...
public class OtpController {
     ...
     * @throws EmailAlreadyRegisteredException if the user with the specified email is already registered
     */
    @Transactional
    @PostMapping("/send")
    public ResponseEntity<Map<String, String>> sendOtp(@RequestBody OtpRequest otpRequest) {
        ...
        // ensure user's email is not already registered with an account
        userService.assertEmailNotAlreadyRegistered(email);
        ...
        if (otpService.emailHasActiveOtp(email)) {
     ...
     *
     * @param verifyOtpRequest an object containing the username and OTP to be verified
    ...
    public ResponseEntity<Map<String, String>> verifyOtp(@RequestBody VerifyOtpRequest verifyOtpRequest) {
        ...
        otpService.deleteOtpByEmail(verifyOtpRequest.getEmail());
